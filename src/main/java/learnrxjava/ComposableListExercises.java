    ...
    */
    public static ComposableList<JSON> exercise12() {
        ...
        return movieLists.concatMap(movieList -> movieList.videos).concatMap(video ->
                video.boxarts.filter(boxArt -> (boxArt.width == 150 && boxArt.height == 200)).map(boxArt -> json("id", video.id, "title", video.title, "boxart", boxArt.url)));
...
//        throw new UnsupportedOperationException("Not implemented yet.");
    ...
    */
    ...
    public static ComposableList<JSON> exercise19() {
        ...
        return movieLists.concatMap(movieList -> movieList.videos).map(video -> {
            ComposableList<BoxArt> reduce = video.boxarts.reduce((b1, b2) -> {
            ...
            return json("id", video.id, "title", video.title, "boxart", reduce.get(0).url);
        });
...
//        throw new UnsupportedOperationException("Not implemented yet.");
    ...
    */
    ...
    public static ComposableList<JSON> exercise22() {
        ...
        return zip(videos, bookmarks, (video, bookmark) -> json("videoId", video.id, "bookmarkId", bookmark.id));
    ...
    public static ComposableList<JSON> exercise23() {
        ...
        return movieLists.concatMap(movieList -> movieList.videos).map(video ->
        {
            BoxArt boxArt = video.boxarts.reduce((ba1, ba2) -> {
                if (ba1.width * ba1.height > ba2.width * ba2.height) {
                    return ba2;
                } else {
                    return ba1;
                }
            ...
            InterestingMoment middle = video.interestingMoments.filter(im -> im.type.equals("Middle")).get(0);
            return json("id", video.id, "title", video.title, "time", middle.time, "url", boxArt.url);
        });
